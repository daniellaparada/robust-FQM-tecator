...
```{r functional boxplots, echo=FALSE}
# First derivative
...
```{r robust est resbox, echo=FALSE}
######################################
# RESIDUAL BOXPLOTS FROM FLM AND FQM
#####################################
par(mfrow=c(1,2))
boxplot(residuos_ROB_LINEAL_tecdatos,col="steelblue", main="Residuals, FLM")
boxplot(residuos_ROB_CUADRA_tecdatos,col="steelblue", main="Residuals, FQM")
par(mfrow=c(1,1))
```

In both cases, we show the common outliers between residuals boxplots and functional boxplots.

```{r robust est resboxhide, results='hide', cache=FALSE, results=FALSE, warning=FALSE, comment=FALSE, message=FALSE}
##########################################
...
```{r robust est atypical curves, echo=FALSE}
######################################
...
wireframe(gamma_rob_CUADRA,scales = list(arrows = FALSE),   
	...
	column.values=tecdatos$t,drape = TRUE,aspect = c(1,1) , 
  	colorkey = TRUE,at=donde, col.regions=colores, main="Parameter estimation, FQM")
```


Comparative beta from FLM and FQM.

```{r robust est betas, echo=FALSE}
par(mfrow=c(1,2))
plot(tecdatos$t, beta_rob_LINEAL, type = "l", lwd=3,xlab = "Wavelength", pch = 10,
     col = "steelblue", ylab = expression(hat(beta)), main=expression(hat(beta)[LIN]))

plot(tecdatos$t, beta_rob_CUADRA  , type = "l", lwd=3,xlab = "Wavelength", pch = 10,
     col = "steelblue", ylab = expression(hat(beta)), main=expression(hat(beta)[QUAD]))
par(mfrow=c(1,1))
```
...
plot( predichos_ROB_LINEAL, residuos_ROB_LINEAL_tecdatos, xlab=expression(hat(y[i])), ylab=expression(r[i]), cex=1.3, cex.lab=1.3, main="FLM fit")
 abline(h=bb_L$stats[1,1],col="red")
abline(h=bb_L$stats[5,1],col="red")

plot( predichos_ROB_CUADRA, residuos_ROB_CUADRA_tecdatos,
xlab=expression(hat(y[i])), ylab=expression(r[i]), cex=1.3, cex.lab=1.3, main="FQM fit")
